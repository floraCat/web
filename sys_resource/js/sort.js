...
/*****************************************************************Sort_排序组 start************/
		...
		$(document).ready(function(){
			...
			$(".return a").click(function(){//恢复默认
					...
					$("#tpl_list_body").attr("src","index.php?col="+col+"&block=list&act=list");
		...
		$(document).ready(function(){
				...
				$(".Sort .btn a").click(function(){
						...
						var col=$("input[name='col']").val();
						if($(this).parent().attr("id")=="sortway"){
						...
						$(".display_sortkey").hide();
								...
								if(sortway_){
									sortway="&sortway="+sortway_+""; 
								}else{sortway="";};						
								/*if($("#sortway a.on").hasClass("opp")){
									sortway="&sortway="+sortway_+"_";
									$("#sortway a.on").removeClass("opp");
								};	*/				

								//列举全部手机端或全部PC端
								var listType_=$(".listType a.on").attr("data-role");
								if(listType_){
									listType="&list="+listType_+""; 
								}else{listType="";};						
								/*if($("#sortway a.on").hasClass("opp")){
									sortway="&sortway="+sortway_+"_";
									$("#sortway a.on").removeClass("opp");
						...
						$("#tpl_list_body").attr("src","index.php?&col="+col+"&block=list&act=list"+grade+""+label+""+mark+""+sortway+""+listType+""+title+""+description+""+content+""+type+""+feature+""+style+""+keywords+""+color+""+format+""+attr+"");
